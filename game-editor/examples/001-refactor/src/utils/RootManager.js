...
class RootManager {
    ...
    createBubbleGameScene(config = {}) {
        // 暂时优先使用 fallback 实现，因为它包含完整的游戏功能
        // SceneManager 的实现还不够完整
        if (this.sceneManager && false) { // 暂时禁用 SceneManager
            const scene = this.sceneManager.createBubbleGameScene(config);
            
            // 确保返回结构与 fallback 一致
            if (scene && scene.gameState && scene.collisionDetector) {
                scene.gameState.collisionDetector = scene.collisionDetector;
            }
    ...
    /**
     * 创建泡泡龙游戏场景的fallback实现
     * @param {Object} config 配置对象
     * @returns {Object} 游戏场景对象
     * @private
     */
    _createBubbleGameSceneFallback(config = {}) {
        console.log('RootManager: 使用 fallback 实现创建游戏场景');
        ...
        const defaultConfig = {
            ...
            collision: {
                checkRadius: 30  // 减小碰撞检测半径，避免过早触发
            },
            initialEggs: {
                rows: 2,
                randomColors: true
        ...
        // 6. 创建游戏状态管理器
        const gameState = this._createBubbleGameStateFallback({
        ...
        if (shooter.updateOnShootCallback) {
            shooter.updateOnShootCallback((projectile, velocity, power, physicsConfig) => {
                ...
                if (physicsConfig.usePhysics && window.physicsManager) {
                    ...
                    console.log('发射物已添加到物理系统和碰撞检测，ID:', projectileId, '活跃发射物数量:', gameState.activeProjectiles.length);
                }
                
                // 准备下一个发射物
                if (gameState.prepareNextProjectile) {
     ...
     * @private
     */
    _calculatePhysicsBoundsFallback(sceneConfig) {
    ...
    /**
     * 创建泡泡龙游戏状态管理器的fallback实现
     * @param {Object} config 配置对象
     * @returns {Object} 游戏状态管理器
     * @private
     */
    _createBubbleGameStateFallback(config) {
    ...
    /**
     * findFloatingEggs代理方法 - 首先尝试GameLogicManager，失败则使用fallback
     */
    findFloatingEggs(gridManager) {
        try {
            // 首先尝试使用GameLogicManager，但要避免 GameLogicManager 中的逻辑错误
            if (this.gameLogicManager && this.gameLogicManager.findFloatingEggs) {
                // 检查 gridManager 是否有自己的 findFloatingEggs 方法
                if (gridManager && typeof gridManager.findFloatingEggs === 'function') {
                    return gridManager.findFloatingEggs();
                } else {
                    // 直接使用我们的 fallback，避免 GameLogicManager 中的递归
                    return this._findFloatingEggsFallback(gridManager);
                }
            }
        } catch (e) {
            console.warn('GameLogicManager.findFloatingEggs failed, using fallback:', e);
        }
    ...
    /**
     * findFloatingEggs的fallback实现
     * @private
     */
    _findFloatingEggsFallback(gridManager) {
        if (!gridManager || !gridManager.grid) {
            console.log('No gridManager or grid available for floating eggs detection');
            return [];
        }
        
        // 回退实现，模仿 GameLogicManager 的逻辑
        const connected = new Set();
        const floatingEggs = [];

        // 标记所有与顶行连接的蛋（使用递归闭包）
        const markConnected = (row, col) => {
            const key = `${row},${col}`; // 使用逗号分隔符，与 GameLogicManager 一致
            const egg = gridManager.grid[row] && gridManager.grid[row][col];
        ...
        // 从顶行开始标记
        const cols = gridManager.cols || gridManager.config.cols;
        const rows = gridManager.rows || gridManager.config.rows;
        
        console.log(`Checking floating eggs in grid: ${rows}x${cols}`);
        ...
        console.log(`Total eggs in grid: ${totalEggs}`);
        ...
        console.log(`Connected eggs: ${connected.size}`);

        // 找出未连接的蛋
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                const egg = gridManager.grid[row] && gridManager.grid[row][col];
                if (egg && !connected.has(`${row},${col}`)) { // 使用逗号分隔符
                    floatingEggs.push({ row, col, egg });
                    console.log(`Found floating egg at ${row},${col}`);
                }
            }
        }

        console.log(`Found ${floatingEggs.length} floating eggs in fallback implementation`);
        return floatingEggs;
    }

    /**
     * clearAllEliminationMarkers代理方法 - 首先尝试GameLogicManager，失败则使用fallback
     */
    clearAllEliminationMarkers(gridManager) {
        try {
            // 首先尝试使用GameLogicManager
            if (this.gameLogicManager && this.gameLogicManager.clearAllEliminationMarkers) {
                return this.gameLogicManager.clearAllEliminationMarkers(gridManager);
            }
        } catch (e) {
            console.warn('GameLogicManager.clearAllEliminationMarkers failed, using fallback:', e);
        }
    ...
    /**
     * clearAllEliminationMarkers的fallback实现
     * @private
     */
    _clearAllEliminationMarkersFallback(gridManager) {
        if (!gridManager || !gridManager.grid) return 0;
        ...
        for (let row = 0; row < gridManager.config.rows; row++) {
            for (let col = 0; col < gridManager.config.cols; col++) {
                const egg = gridManager.grid[row] && gridManager.grid[row][col];
                if (egg && egg.markedForElimination) {
                    delete egg.markedForElimination;
                    clearedCount++;
                }
            }
        }
        
        console.log(`Cleared ${clearedCount} elimination markers`);
        return clearedCount;
    }
     ...
     * @param {number} minMatchCount - 最小匹配数量（默认3）
     * @returns {Object} 匹配结果 { matched: boolean, eggs: Array }
     */
    checkAndMarkMatches(gridManager, startRow, startCol, minMatchCount = 3) {
        if (!gridManager || !gridManager.grid) return { matched: false, eggs: [] };
        
        const targetColor = gridManager.grid[startRow][startCol]?.eggGameColor;
        if (!targetColor) return { matched: false, eggs: [] };
        ...
        // 标记匹配的蛋
        if (matchedEggs.length >= minMatchCount) {
            matchedEggs.forEach(egg => {
                egg.markedForElimination = true;
            });
            return { matched: true, eggs: matchedEggs };
        } else {
            return { matched: false, eggs: [] };
        }
    }

    /**
     * 消除标记的蛋并返回消除的蛋的数组
     * @param {Array} markedEggs - 被标记的蛋的数组
     * @param {GridManager} gridManager - 网格管理器实例
     * @returns {Array} 消除的蛋的数组
     */
    eliminateMarkedEggs(markedEggs, gridManager) {
        if (!gridManager || !gridManager.grid) return [];
        
        const eliminatedEggs = [];
        
        markedEggs.forEach(egg => {
            for (let row = 0; row < gridManager.config.rows; row++) {
                for (let col = 0; col < gridManager.config.cols; col++) {
                    const gridEgg = gridManager.grid[row] && gridManager.grid[row][col];
                    if (gridEgg === egg) {
                        // 从网格中移除蛋
                        gridManager.grid[row][col] = null;
                        eliminatedEggs.push(egg);
                        console.log(`消除蛋: (${row}, ${col}) 颜色: ${egg.eggGameColor.toString(16)}`);
                        break;
                    }
                }
            }
